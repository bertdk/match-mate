...
export class TransformInterceptor extends ClassSerializerInterceptor {
  override intercept(
    ...
    const responseMetadata = Reflect.getMetadata(
      'swagger/apiResponse', // TODO: fix with additional plugin, swagger doesn't really match the needs here
      handlerFunc
    );
    const responseType = (
      Object.values(responseMetadata)?.find((m: any) => m.type) as any
    )?.type;
